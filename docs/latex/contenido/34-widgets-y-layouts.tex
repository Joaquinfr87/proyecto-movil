  ...
  \contenidoConFuente{%
    \begin{tablaAPA}
      \begin{tabular}{@{}p{3.4cm}p{4.0cm}p{8.2cm}@{}}
        \toprule        \textbf{Widget Flutter} & \textbf{Equivalente React Native} &
        \textbf{Uso en el proyecto} \\midrule        \textit{TextField} & \textit{TextInput} & Campos de formulario
        (email, contrase\~na, nombre) en las pantallas de login y registro. \\addlinespace        \textit{Text} & \textit{Text} & T\'itulos, subt\'itulos, mensajes,
        etiquetas y textos informativos en todas las pantallas. \\addlinespace        \textit{ElevatedButton} & \textit{TouchableOpacity} & Botones de
        acci\'on: \textit{``Comenzar''}, \textit{``Iniciar sesi\'on''},
        \textit{``Crear cuenta''}, \textit{``Cerrar sesi\'on''}. \\addlinespace        \textit{Column} & \textit{View} con \textit{flexDirection: 'column'} &
        Organizaci\'on vertical de elementos en todas las pantallas. \\addlinespace        \textit{Row} & \textit{View} con \textit{flexDirection: 'row'} &
        Disposici\'on horizontal: barra de navegaci\'on, footer de enlaces,
        bot\'on de logout con icono. \\addlinespace        \textit{Padding} & \textit{padding*} en \textit{StyleSheet} &
        Espaciado interno de contenedores, campos y botones. \\addlinespace        \textit{Container} & \textit{View} con estilos de fondo y borde &
        Contenedores de avatar, campos de formulario y cajas de mensajes. \\addlinespace        -- & \textit{KeyboardAvoidingView} & Ajuste autom\'atico de la
        interfaz cuando aparece el teclado virtual. \\addlinespace        -- & \textit{ScrollView} & Desplazamiento del contenido cuando
        excede el tama\~no de la pantalla. \\addlinespace        -- & \textit{ActivityIndicator} & Indicador de carga durante el
        env\'io de formularios (login y registro). \\addlinespace        -- & \textit{Alert} & Di\'alogo de confirmaci\'on para el cierre
        de sesi\'on. \\bottomrule
      \end{tabular}
    \end{tablaAPA}%
