...
\begin{table}[H]
  ...
  \contenidoConFuente{%
    \begin{tablaAPA}
      \begin{tabular}{@{}p{1.0cm}p{6.5cm}p{1.8cm}p{2.5cm}p{2.5cm}@{}}
        \toprule
...
\begin{figure}[H]
\centering
  \figuraAPA{fig:mapa-historias}{Mapa de historias de usuario por módulo}{%
  \begin{tikzpicture}[
      modulo/.style={rectangle, draw=black!60, rounded corners=2pt,
                     align=center, fill=blue!8, font=\tiny,
      ...
      historia/.style={rectangle, draw=black!40, rounded corners=1pt,
                       ...
                       text width=2.4cm, minimum height=0.4cm},
      flecha/.style={-{Stealth[length=1.5mm]}, thick, draw=black!40}
    ]
    \node[modulo] (auth) at (0,3.2) {Autenticación\\(HU-01 a HU-04)};
    \node[modulo] (expl) at (-5.0,1.0) {Exploración\\(HU-05 a HU-11)};
    \node[modulo] (fav) at (5.0,1.0) {Favoritos\\(HU-12 a HU-14)};
    \node[modulo] (gest) at (-5.0,-1.2) {Gestión\\(HU-15 a HU-17)};
    \node[modulo] (ui) at (5.0,-1.2) {Interfaz\\(HU-18 a HU-22)};

    \draw[flecha] (auth) -- (expl);
    \draw[flecha] (auth) -- (fav);
    \draw[flecha] (expl) -- (gest);
    \draw[flecha] (fav) -- (ui);
  \end{tikzpicture}%
  }{Elaboración propia.}
\end{figure}
