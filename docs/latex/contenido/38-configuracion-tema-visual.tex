  ...
  \contenidoConFuente{%
    \begin{tablaAPA}
      \begin{tabular}{@{}p{2.2cm}p{2.6cm}p{2.2cm}p{8.6cm}@{}}
        ...
        \textbf{Uso} \\\
        \midrule
        \textbf{Primario} & \texttt{primary} & \texttt{\#2563EB} & Color
        principal de la marca: botones, enlaces, \'iconos activos. \\\
        \addlinespace
        Primario & \texttt{primaryDark} & \texttt{\#1D4ED8} & Fondo de la
        parte superior del splash. \\\
        \addlinespace
        Primario & \texttt{primaryLight} & \texttt{\#93C5FD} & Texto tagline
        del splash, elementos secundarios. \\\
        \addlinespace
        \textbf{Secundario} & \texttt{secondary} & \texttt{\#10B981} & Color
        de \'exito, confirmaciones positivas. \\\
        \addlinespace
        \textbf{Superficie} & \texttt{background} & \texttt{\#F8FAFC} &
        Fondo general de todas las pantallas. \\\
        \addlinespace
        Superficie & \texttt{surface} & \texttt{\#FFFFFF} & Fondo de
        tarjetas, campos de formulario y botones. \\\
        \addlinespace
        \textbf{Texto} & \texttt{text} & \texttt{\#0F172A} & Color principal
        del texto (t\'itulos, contenido). \\\
        \addlinespace
        Texto & \texttt{textSecondary} & \texttt{\#64748B} & Texto
        secundario, subt\'itulos y placeholders. \\\
        \addlinespace
        \textbf{Estado} & \texttt{error} & \texttt{\#EF4444} & Errores de
        validaci\'on y mensajes de error del servidor. \\\
        \addlinespace
        Estado & \texttt{success} & \texttt{\#22C55E} & Mensajes de
        \'exito (registro exitoso). \\\
        \addlinespace
        Estado & \texttt{warning} & \texttt{\#F59E0B} & Advertencias
  ...
  \contenidoConFuente{%
    \begin{tablaAPA}
      \begin{tabular}{@{}p{3.2cm}p{3.6cm}p{8.8cm}@{}}
        ...
        \midrule
        \texttt{fontSize.xs} & 12\,pt & Mensajes de error y pie de figura. \\\
        \addlinespace
        \texttt{fontSize.sm} & 14\,pt & Etiquetas de campo y texto secundario. \\\
        \addlinespace
        \texttt{fontSize.md} & 16\,pt & Texto de cuerpo y contenido principal. \\\
        \addlinespace
        \texttt{fontSize.lg} & 18\,pt & Texto de botones. \\\
        \addlinespace
        \texttt{fontSize.title} & 32\,pt & T\'itulos de pantalla. \\\
        \addlinespace
        \texttt{spacing.md} & 16\,pt & Espaciado est\'andar entre
        elementos. \\\
        \addlinespace
        \texttt{borderRadius.md} & 8\,pt & Radio de bordes de campos y
        botones. \\\
        \bottomrule
      \end{tabular}
    \end{tablaAPA}%
