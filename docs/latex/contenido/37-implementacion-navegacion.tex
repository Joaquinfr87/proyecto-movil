...
\begin{figure}[H]
  \centering
  \caption{Diagrama de navegaci\'on implementado en la aplicaci\'on \nombreApp{}}
  \label{fig:navegacion-implementada}
  \contenidoConFuente{%
  \begin{tikzpicture}[
      ...
      stack/.style={rectangle, draw=blue!60, rounded corners=3pt,
                    dashed, inner sep=8pt, fill=blue!3},
      tab/.style={rectangle, draw=green!60, rounded corners=3pt,
                  dashed, inner sep=8pt, fill=green!3},
      flecha/.style={-{Stealth[length=2.5mm]}, thick}
    ]
    % Stack Navigator
    \node[stack, fit={(0,0) (7,3.5)}, label={[font=\small\bfseries]above:Stack Navigator}] (stack) {};
    \node[nodo, fill=yellow!15] (splash) at (3.5,2.5) {Splash};
    \node[nodo] (login) at (1.5,1.2) {Login};
    \node[nodo] (register) at (5.5,1.2) {Registro};
    \node[nodo] (detail) at (5.5,0.0) {Scenario [id]};

    % Tab Navigator
    \node[tab, fit={(10,-1.5) (16.5,3.5)}, label={[font=\small\bfseries]above:Tab Navigator}] (tabs) {};
    \node[nodo] (home) at (11.2,2.5) {Inicio};
    \node[nodo] (search) at (14.8,2.5) {Buscar};
    \node[nodo] (fav) at (11.2,0.7) {Favoritos};
    \node[nodo] (profile) at (14.8,0.7) {Perfil};

    \draw[flecha] (splash) -- (login);
    \draw[flecha] (splash) -- (register);
    \draw[flecha] (login) -- (register) node[midway, below, font=\scriptsize] {Registrate};
    \draw[flecha] (login) -- (home) node[midway, above, font=\scriptsize] {Autenticar};
    \draw[flecha] (home) -- (detail);
    \draw[flecha, dashed] (profile) to[bend right=30] node[below, font=\scriptsize] {signOut} (login);
  \end{tikzpicture}%
