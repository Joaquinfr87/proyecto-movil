...
\textbf{Rama:} \texttt{ExamenFinalNicolasReguerin}\\[0.5em]
\today
\end{center}
...
\href{https://github.com/Joaquinfr87/proyecto-movil/tree/ExamenFinalNicolasReguerin}
     {github.com/Joaquinfr87/proyecto-movil/tree/ExamenFinalNicolasReguerin}

\section{Funcionalidad implementada}
\label{sec:funcionalidad}

Los usuarios autenticados pueden calificar cada escenario deportivo con 1 a 5 estrellas
y dejar un comentario opcional. La aplicación permite:

\begin{itemize}
  \item Ver el promedio y la cantidad de valoraciones de cada escenario
        (en el detalle y en las tarjetas del catálogo).
  \item Ver la lista de reseñas con el autor y su comentario.
  \item Gestionar desde la pestaña \emph{Valoraciones} las mis valoraciones
...
\begin{itemize}
  \item \emph{Ciudadanía} (rol \texttt{asistente} / cualquier usuario autenticado): valora,
        consulta y gestiona sus propias reseñas.
  \item \emph{Gestores y administradores}: consultan las valoraciones de los escenarios que
...
\begin{center}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{4.6cm}@{}}
\toprule
...
\texttt{src/app/scenario/[id].tsx} & Modificada & Sección Valoraciones: promedio, botón Valorar, reseñas\\
...
\midrule
id & UUID PK & \normalfont\texttt{gen\_random\_uuid()}\\
scenario\_id & UUID FK $\rightarrow$ \texttt{scenarios.id} & \normalfont\texttt{ON DELETE CASCADE}\\
user\_id & UUID FK $\rightarrow$ \texttt{profiles.id} & \normalfont\texttt{ON DELETE CASCADE}\\
rating & SMALLINT & \normalfont\texttt{CHECK (rating BETWEEN 1 AND 5)}\\
comment & TEXT & \normalfont default \texttt{''}, máx.\ 500 (validación en app)\\
created\_at / updated\_at & TIMESTAMPTZ & \normalfont trigger automático\\
\bottomrule
\end{tabularx}
\end{center}

\emph{Restricción única:} \texttt{UNIQUE (user\_id, scenario\_id)} -- una valoración por usuario
y escenario. \emph{Índices:} por \texttt{scenario\_id} y por \texttt{user\_id}.

\subsection{RLS (Row Level Security)}
\begin{center}
\begin{tabularx}{0.8\textwidth}{@{}>{\ttfamily}lX@{}}
\toprule
\textbf{\normalfont Operación} & \textbf{\normalfont Política}\\
\midrule
SELECT & \normalfont autenticados y anónimos (el público puede ver reseñas)\\
INSERT & \normalfont\texttt{user\_id = auth.uid()}\\
UPDATE & \normalfont\texttt{user\_id = auth.uid()}\\
DELETE & \normalfont\texttt{user\_id = auth.uid()} o rol \texttt{admin}\\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{center}
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}p{4.2cm}X@{}}
\toprule
...
\begin{itemize}
  \item \emph{Borrador de la valoración} (\texttt{rating\_draft:v1:\{userId\}}): si el usuario
        escribe estrellas / comentario y cierra el modal sin guardar, el borrador se restaura la
        próxima vez.
  \item \emph{Caché offline de ``Mis Valoraciones''} (\texttt{my\_ratings\_cache:v1:\{userId\}}):
...
\begin{itemize}
  \item \emph{Validación de campos:} Zod en el formulario -- calificación requerida entre 1 y 5;
        comentario opcional con máximo 500 caracteres.
  \item \emph{Datos inválidos:} el \texttt{CHECK (rating BETWEEN 1 AND 5)} y el
        \texttt{UNIQUE} refuerzan a nivel de base de datos.
  \item \emph{Información inexistente:} \texttt{EmptyState} (``Sin valoraciones aún'') y estado
        de error con reintento.
  \item \emph{Error al guardar:} mensaje en el modal sin cerrar el formulario.
  \item \emph{Error de consulta:} \texttt{ErrorState} con botón ``Reintentar'' en la pestaña.
  \item \emph{Doble envío:} botón deshabilitado con \emph{spinner} mientras guarda.
\end{itemize}
...
\begin{center}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.8cm}X>{\raggedright\arraybackslash}p{4.4cm}@{}}
\toprule
...
Las pruebas 1--12 se ejecutaron contra \emph{Supabase local}, con un script de
\emph{smoke test} usando \texttt{@supabase/supabase-js}: autenticación con usuarios
\emph{seed}, CRUD completo, \emph{constraints} y control de RLS.
...
\begin{enumerate}
  \item \emph{Privilegios de tabla en Supabase local:} las tablas nuevas no recibían los grants
        automáticos (\texttt{authenticated} sin INSERT/SELECT); se agregaron \texttt{GRANT}
        explícitos en la migración.
  \item \emph{Caché de esquema de PostgREST:} tras aplicar la migración la API devolvía 404
        hasta recargar el esquema (resuelto con \texttt{db reset} y reinicio del contenedor
        \texttt{rest}).
  \item \emph{Ambigüedad en el listado de migraciones local} que sugería que \texttt{011} estaba
        aplicada cuando no lo estaba; verificado directamente con \texttt{psql}.
  \item \emph{Tipos de expo-router desactualizados} (\texttt{.expo/types/router.d.ts}):
        errores de TS preexistentes en \texttt{manage.tsx} que se regeneran con \texttt{expo start}.
  \item \emph{Lint preexistente roto} en el repo (ESLint v10 requiere \emph{flat config} y el repo
...
Para correrla en un celular conectado por USB a la PC:

\begin{enumerate}
  \item Conectar el celular por USB y habilitar la \emph{depuración por USB}.
  \item Verificar la IP de la base: si se usa \emph{Supabase local}, apuntar en \texttt{.env} a la
  ...
  \item Compilar e instalar directamente en el dispositivo conectado:

\begin{verbatim}
pnpm android
\end{verbatim}

Este comando (\texttt{expo run:android}) compila, instala y lanza la app en el celular.

  \item Para generar solo el APK firmado (instalable manualmente), compilar en modo
        \texttt{release} (firma con la \emph{debug key}):
