...
export function WebMapLibre({ scenarios, apiKey, onScenarioSelect }: WebMapLibreProps) {
  const mapContainerRef = useRef<any>(null);
  const mapInstanceRef = useRef<any>(null);
  const markersRef = useRef<any[]>([]);

  useEffect(() => {
    // 1. Cargar CSS de MapLibre GL si no está presente
    ...
    if (window.maplibregl) {
      initMap();
    } else if (!document.getElementById('maplibre-gl-js')) {
  ...
  useEffect(() => {
    ...
    if (!map || !window.maplibregl || !scenarios || scenarios.length === 0) return;
    ...
    scenarios.forEach((sc) => {
      ...
      // Crear elemento HTML personalizado para el pin
